...
export class FrameCropper {
  ...
  private transformFrame(
    frame: ParsedFrame,
    frameImgData: ImageData | undefined
  ): void {
  ...
  private setupCanvas() {
    const containerCanvas = (this.containerCanvas =
      document.createElement('canvas'))
    const convertCanvas = (this.convertCanvas =
      document.createElement('canvas'))
  ...
  private setCanvasWH() {
    ...
    this.containerCtx.clearRect(
      0,
      0,
      this.containerCanvas.width,
      this.containerCanvas.height
    )
